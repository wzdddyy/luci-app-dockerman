...
function get_containers()
    ...
    for i, v in ipairs(containers) do
        -- 安全检查：确保容器数据完整，避免nil值导致的崩溃
        if not v or not v.Id or not v.Status then
            -- 跳过数据不完整的容器条目
        else
            local index = (10^12 - (v.Created or 0)) .. "_id_" .. v.Id
            ...
            local container_name = v.Names and v.Names[1] and v.Names[1]:sub(2) or v.Id:sub(1,12)

            if v.Status:find("^Up") then
            data[index]["_name"] = "<font color='green'>"..container_name.."</font>"
            ...
            data[index]["_name"] = "<font color='red'>"..container_name.."</font>"
        ...
        if (type(v.NetworkSettings) == "table" and type(v.NetworkSettings.Networks) == "table") then
            for networkname, netconfig in pairs(v.NetworkSettings.Networks) do
                -- 安全检查：确保netconfig和IPAddress都存在
                local ip_addr = netconfig and netconfig.IPAddress or ""
                data[index]["_network"] = (data[index]["_network"] ~= nil and (data[index]["_network"] .." | ") or "").. networkname .. (ip_addr ~= "" and (": " .. ip_addr) or "")
            end
        end
        ...
        if v.Ports and next(v.Ports) ~= nil then
            ...
            for _,v2 in ipairs(v.Ports) do
                local ip_addr = ip.new(v2.IP or "0.0.0.0")
                if ip_addr and ip_addr:is4() then
        ...
        for ii,iv in ipairs(images or {}) do
            -- 安全检查：确保v.ImageID存在，避免与nil值比较导致崩溃
            if v.ImageID and iv.Id == v.ImageID then
        ...
        if type(v.Mounts) == "table" and next(v.Mounts) then
            for _, v2 in pairs(v.Mounts) do
                -- 安全检查：跳过挂载类型不正确或数据不完整的挂载条目
                if v2.Type ~= "volume" and v2.Source and v2.Destination then
        ...
        data[index]["_image_id"] = v.ImageID and v.ImageID:sub(8,20) or ""
        data[index]["_command"] = v.Command or ""
        end
    end
    return data
end
